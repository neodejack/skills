---
name: build-from-exec-plan-hitl
...
1. Resolve the target plan.
   - Expect requests like `use the build-from-exec-plan-hitl skill to build <plan_name>`.
...
6. Pause for human review before committing each milestone.
   - After a milestone is completed and verified, summarize the milestone for human review before committing.
   - Keep the review message concise. Include only:
     - what was completed in the milestone
     - verification run and result
     - important decision log entries
     - notable surprises or discoveries
     - any blockers, validation gaps, or follow-up concerns
   - Ask the human/user to review the milestone work.
   - If the human/user requests changes, keep working on the same milestone. Re-verify the affected work and present an updated concise review summary.
   - Proceed to the commit step only after explicit human approval, such as `approved`, `commit it`, or equivalent clear approval.
   - Do not treat silence, ambiguous feedback, or partial review comments as approval.

7. Commit after every completed milestone.
   - Create a git commit immediately after each milestone is completed, verified, and explicitly approved by the human/user.
...
- which milestone review gates were approved
